-- Script SQL para crear perfiles de talento de prueba
-- Ejecutar en Supabase SQL Editor

-- Nota: Estos UUIDs son generados aleatoriamente y únicos
-- Las contraseñas se establecerán como "CastingfyDemo2025!" (hash bcrypt)
...
INSERT INTO auth.users (
  ...
  '$2a$10$5IH8h8h8h8h8h8h8h8h8h8h8h8h8h8h8h8h8h8h8h8h8h8h8', -- Contraseña: CastingfyDemo2025!
  NOW(),
  NOW(),
  NOW(),
  'authenticated',
...
-- Insertar en users y talent_profiles para María García
DO $$
DECLARE
  user_id uuid;
BEGIN
  SELECT id INTO user_id FROM auth.users WHERE email = 'maria.garcia@example.com';

  IF user_id IS NOT NULL THEN
    INSERT INTO users (id, email, role, status)
    VALUES (user_id, 'maria.garcia@example.com', 'talento', 'verified')
    ON CONFLICT (id) DO NOTHING;

    INSERT INTO talent_profiles (user_id, stage_name, location, age, gender, height, bio, headshot_url, languages, skills)
    VALUES (
      user_id,
      'María García',
      'Madrid, España',
...
-- Repetir para los demás talentos usando una función temporal
CREATE OR REPLACE FUNCTION create_test_talent(
  p_email text,
  p_stage_name text,
  p_location text,
  p_age integer,
...
BEGIN
  ...
  -- Insertar o actualizar perfil de talento
  INSERT INTO talent_profiles (
    user_id, stage_name, location, age, gender, height, bio,
    headshot_url, languages, skills
  )
  VALUES (
    v_user_id, p_stage_name, p_location, p_age, p_gender, p_height, p_bio,
    p_headshot_url, p_languages, p_skills
  )
  ON CONFLICT (user_id) DO UPDATE SET
    stage_name = EXCLUDED.stage_name,
    location = EXCLUDED.location,
...
-- Crear todos los talentos de prueba
SELECT create_test_talent(
...
DROP FUNCTION IF EXISTS create_test_talent;
